...
def test_wilcoxon_paired_identical_series_returns_p_equals_1():
    ...
    stat, p, n, sign, mean_diff, median_diff = _wilcoxon_paired(base, same)
    assert n == 20
    assert p == 1.0
    # All diffs zero -> sign must be 0 (no direction), both diff
    # magnitudes = 0.
    assert sign == 0
...
def test_wilcoxon_paired_significant_when_strictly_larger():
    ...
    stat, p, n, sign, mean_diff, median_diff = _wilcoxon_paired(
        (base_vals, users), (mod_vals, users))
    assert n == 60
    assert p < 0.01
    # Model wins on every user -> sign=+1, both diff magnitudes =+0.05.
    assert sign == 1
...
def test_wilcoxon_paired_intersects_on_user_id():
    ...
    stat, p, n, sign, mean_diff, median_diff = _wilcoxon_paired(base, mod)
